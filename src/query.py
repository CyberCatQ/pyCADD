...
try:
    list_dir_path = lib_path + 'list/'                                  # 记录基因PDBID的列表文件库
...
try:
    ...
    os.makedirs(bdaffinity_dir_path)
except FileExistsError:
    pass
...
与RCSB PDB服务器交互 获取基因相关晶体信息
    ...
    def __precess_inputfile(self):
        '''
        解析输入文件
        '''

        gene_file = sys.arg[1]      # 从命令行解析提供的基因列表文件
    ...
    def query(self, gene_list=[]):
        ...
        for gene in gene_list:
            ...
            with open(list_dir_path + gene + '.txt', 'r') as f:
                pdbs = f.readlines()
                    ...
                    query = '''\
{
    ...
    def convert_csv(self, gene_list=[]):
        ...
        for gene in gene_list:
            ...
            jsfile = open(json_dir_path + gene, 'r')
            ...
            for dic_pdb in lis_data:
                ...
                if d['binding_affinity']:
                    with open('BDaffinity/' + d['PDBID'] + '_binding_affinity.csv', 'w') as f:
    ...
    def make_inputfile(self, gene_list=[]):
        ...
        for gene in gene_list:
            ...
            f = open(csv_file, 'r')
            inputfile = open(list_dir_path + gene + '.txt', 'w')
            reader = csv.reader(f)
