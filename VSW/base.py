...
class VSW(Multidock):
    ...
    def genelist(self) -> list:
        ...
        基因成员列表
        '''
        if not self.receptor_config:
            raise ValueError('No receptor config loaded.')
        return list(self.receptor_config.keys())
    ...
    def read_gene(self, gene_config_path) -> str:
        '''
        读取受体信息
        '''
        logger.info('Reading gene info from %s' % gene_config_path)
        self.gene_config = core.read_gene_config(gene_config_path)
        logger.info('Loaded Receptor Info.')

    def read_databse(self, database_config_path) -> str:
        '''
        读取化合物库信息
        '''
    ...
    def get_gene(self) -> str:
        ...
        self.gene = Prompt.ask('Enter the required gene:', choices=self.genelist)
        return self.gene
