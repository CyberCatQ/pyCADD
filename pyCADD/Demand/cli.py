...
@click.option(
    "--output_format",
    "-o",
    type=click.Choice(["csv", "in", "ini", "yml", "yaml"]),
    default="yml",
    help="Output format.",
)
@click.option(
    "-f", "--pdb_list_file", type=click.Path(exists=True), help="CSV file with pdb list to query."
)
@click.option(
    "--pdb_column",
    "-p",
    type=str,
    default="PDB",
    help="Column name of pdb id in pdb list file. Default by 'PDBID' if not specified.",
)
...
@click.option(
    ...
    "-m",
    is_flag=True,
    help="DONT Delete mutated(not wide type) crystal when generate input file for pyCADD-Dock.",
)
@click.option(
    "--not_del_ignore",
    "-e",
    is_flag=True,
    help="DONT Delete small molecule which is not ligand (e.g. solvent molecules) when generate input file for pyCADD-Dock",
)
def main(
    uniprot_id,
    not_del_mutations,
