...
def calc_am1bcc(
    ...
    Note:
        ...
        The output file is named "{input_prefix}_am1bcc.mol2".
    """
    ...
    mol2_file_path = os.path.join(save_dir, f"{file_prefix}_am1bcc.mol2")
    ...
    shell_run(
        ...
        f"-c bcc -nc {charge} -m {multiplicity} -at gaff2"
    )
...
def run_parmchk2(
    ...
    Args:
        mol2_file_path (str): Path to MOL2 file with assigned atom types (from antechamber).
    ...
    cmd = f"parmchk2 -i {mol2_file_path} -f mol2 -o {frcmod_file_path}"
    shell_run(cmd)
