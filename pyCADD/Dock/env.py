...
def create_environment(environment_name, schrodinger_home, root_path=None):
    ...
    try:
        subprocess.run(f"{schrodinger_home}/run schrodinger_virtualenv.py {env_path}", shell=True, check=True, stdout=subprocess.DEVNULL)
        subprocess.run(f"bash -c 'source {env_path}/bin/activate && pip install -e {root_path}'", shell=True, check=True, stdout=subprocess.DEVNULL)
...
def main():
    ...
    if not check_virtual_environment(package_name, environment_name):
        ...
        subprocess.run(f"bash -c 'source {env_path}/bin/activate > /dev/null && pycadd-dock {args}'", shell=True, check=True)
        sys.exit(0)
